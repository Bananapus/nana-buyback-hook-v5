...
contract DebugSlippage is Test {
    ...
    function testSlippageCalculation() public {
        // Test with the values from the fork test
        uint256 amountIn = 1 ether; // 1 ETH
        uint128 liquidity = 10_000_000 ether; // 10M ETH liquidity (from test setup)
        ...
        testDirection(amountIn, liquidity, sqrtPriceX96, true); // zeroForOne = true
        testDirection(amountIn, liquidity, sqrtPriceX96, false); // zeroForOne = false
    }

    function testDirection(uint256 amountIn, uint128 liquidity, uint160 sqrtP, bool zeroForOne) internal {
        console.log("\n=== Testing direction zeroForOne:", zeroForOne);

        // Base calculation: 2 * 10_000 * (amountIn / liquidity)
        uint256 base = (amountIn * 2 * TWAP_SLIPPAGE_DENOMINATOR) / uint256(liquidity);
        console.log("Base (before sqrtP normalization):", base);
        console.log("Base in %:", (base * 100) / TWAP_SLIPPAGE_DENOMINATOR);

        // Final slippage tolerance calculation
        uint256 slippageTolerance;
        if (zeroForOne) {
            slippageTolerance = (base * uint256(sqrtP)) / (uint256(1) << 96);
        } else {
            slippageTolerance = (base * (uint256(1) << 96)) / uint256(sqrtP);
        }

        console.log("Slippage tolerance (raw):", slippageTolerance);
