...
contract Deploy is Script {
    function run() public {
        ...
        address directoryAddress = _getDeploymentAddress(
            string.concat("juice-contracts-v4/broadcast/Deploy.s.sol/", chain, "/run-latest.json"), "JBDirectory"
        );

        address controllerAddress = _getDeploymentAddress(
            string.concat("juice-contracts-v4/broadcast/Deploy.s.sol/", chain, "/run-latest.json"), "JBController"
        );
